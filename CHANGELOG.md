...
- **修复**：月视图的日程芯片会被浏览器挤到网格外。根因是格子写成了 `<button>`，而芯片本身也是 `<button>` —— HTML 不允许按钮嵌套按钮，解析时外层按钮被提前闭合。格子改用 `<div role="button" tabIndex=0>`（键盘 Enter/空格仍可新建），并加了回归测试：月视图里不允许出现 `button`。
