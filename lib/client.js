...
const CSS = [
  ...
  ".dshc-cell{min-height:88px;border-radius:10px;padding:5px 5px 6px;background:var(--dsw-alias-bg-layer-2,rgba(0,0,0,.03));border:1px solid transparent;cursor:pointer;text-align:left;font:inherit;color:inherit;transition:background .12s,border-color .12s;overflow:hidden}",
...
const UI = {
  zh: {
    ...
    "conn.intro": "填完点「测试连接」，通了才会保存。密码只存在本机 settings.yaml（secret 字段：不进日志、不进导出）。",
    "conn.provider": "服务商",
    ...
    "conn.storageFallback": "宿主 settings 服务不可用（{reason}），配置改存插件自己的文件：~/.dsh/data/dsh-calendar/connection.json —— 保存与读取照常工作。",
  ...
  en: {
    ...
    "conn.intro": "Fill this in and hit「Test connection」— nothing is saved until it works. The password stays in your local settings.yaml as a secret field (no logs, no exports).",
    ...
    "conn.storageFallback": "The host settings service is unavailable ({reason}); the connection is kept in the plugin's own file (~/.dsh/data/dsh-calendar/connection.json) — saving and reading work as usual.",
  ...
  const body = h("div", null, [
    ...
    maySave !== true
      ...
      : (connection !== null && connection !== undefined && connection.settingsKind === "file"
          ? h("div", { className: "dshc-alert" }, t("conn.storageFallback", { reason: connection.settingsReason || "宿主没有提供 settings 服务" }))
          : null),
    h("div", { className: "dshc-field" }, [
    ...
    maySave === true ? null : h("div", { className: "dshc-sub", style: { marginTop: "8px" } }, t("conn.current", { summary: connectionSummary(connection) }))
  ]);
