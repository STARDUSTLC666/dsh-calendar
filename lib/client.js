...
function WeekView(props) {
  ...
  const stopDrag = (commit) => {
    ...
    if (commit !== false && moved === true && canEdit === true) {
      ...
      // 没有幽灵块就没有落点：比如纵向拖出本周（跨到下周）时不该盲提交。
      const visible = days.some((day) => dayKey(day) === slot.dayKey) && ghost !== null;
      if (visible === true) onReschedule(drag.event, rangeOfSlot(slot), drag.mode);
    }
  };
  ...
  const stopAutoScroll = () => {
    if (autoScroll.current.raf !== 0) { cancelAnimationFrame(autoScroll.current.raf); autoScroll.current.raf = 0; }
  };
  const stepAutoScroll = () => {
    ...
    autoScroll.current.raf = requestAnimationFrame(stepAutoScroll);
  };
  const ensureAutoScroll = (pointerEvent) => {
    ...
    if (nearEdge === true && autoScroll.current.raf === 0) autoScroll.current.raf = requestAnimationFrame(stepAutoScroll);
  };
