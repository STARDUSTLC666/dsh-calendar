...
function registerLayerEscape(root, closeTop) {
  ...
  if (layerEscapeListener === null) {
    layerEscapeListener = (event) => {
      ...
      const isGlobalTarget = target === document.body || target === document.documentElement || target === document;
      const root = panelRootOf(target);
      // 焦点在上层宿主 Portal（不在任何日历 root 里）时，把 Esc 还给宿主。
      if (isGlobalTarget !== true && root === null) return;
      let owner = null;
      if (root !== null) {
...
function CalendarPanel(props) {
  ...
  useEffect(() => {
    ...
    if (root === null) return () => {};
    const closeTop = () => {
      if (draft !== null) { setDraft(null); setFormError(null); }
      else if (connOpen === true) setConnOpen(false);
      else if (selected !== null) { setSelected(null); setConfirmDelete(false); }
    };
  ...
  return h("div", { ref: rootRef, className: "dshc-root", "data-dshc-layer": layerOpen === true ? "1" : undefined }, [
