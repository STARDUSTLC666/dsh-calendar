...
export function resolveCredentials(config, env = process.env) {
    ...
    if (username === undefined) {
        throw new ConfigError('日历账号未填写：请在日历的“连接设置”中填写“账号”（iCloud 使用 Apple ID 邮箱），然后重新测试连接。');
    }
    if (password === undefined) {
        throw new ConfigError('日历密码未填写：请在“连接设置”中填写“密码 / 应用专用密码”，然后重新测试连接。iCloud 需要应用专用密码。');
    }
