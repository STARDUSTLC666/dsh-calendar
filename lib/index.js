    ...
    if (typeof ctx.inject === 'function') {
        try {
            ctx.inject(['settings'], (settingsCtx) => {
                if (settingsCtx === undefined || settingsCtx.settings === undefined || typeof settingsCtx.effect !== 'function')
                    return;
                settingsCtx.effect(() => {
                    ...
                    const descriptor = () => (settingsCtx.settings.describe?.() ?? []).find((row) => row.ns === SETTINGS_NAMESPACE);
                    const readRaw = () => (scope.get() ?? {});
                    const read = () => readRaw();
                    // 有效配置 = patch 行配置 + 面板真的设过的字段（面板优先，未设的字段由 YAML 兜底）。
                    const merge = (value) => ({ ...cfg, ...toCalendarConfig(value, descriptor()?.user) });
                    ...
                    backend.attachSettings({
                        ...
                        replace: async (value, revision) => {
                            ...
                            effective = { ...cfg, ...toCalendarConfig(value, null) };
                        },
                    });
                    return () => {
