 ...
 */
export function settingsProviderOf(ctx: any): any | undefined {
  let provider: any
  try { provider = typeof ctx?.get === 'function' ? ctx.get('settings') : undefined } catch { provider = undefined }
  if (provider === undefined || provider === null) {
    try { provider = ctx?.settings } catch { provider = undefined }
  }
  return provider === undefined || provider === null ? undefined : provider
}

/** 0.1.6 及更早：插件在加载阶段注册自己的命名空间。 */
export function isLegacyProvider(provider: any): boolean {
 ...
 */
export function isFormsProvider(provider: any): boolean {
...
export function entryIdOf(ctx: any, fallback: string): string {
...
export async function importLegacySettings(
  ...
  if (isLegacyProvider(provider) || config.legacySettingsImported === true) return false
  if (typeof provider?.update !== 'function') return false
  // 老命名空间是全局的：别把凭据复制进同插件的自定义实例。
  if (entryIdOf(ctx, entryId) !== entryId) return false
  ...
  for (const file of ['settings.yaml.imported', 'settings.yaml']) {
    try {
      ...
      if (error?.code !== 'ENOENT') throw new Error('Cannot read legacy settings; original file preserved')
    }
  }
  if (!plain(document) || !plain(document[namespace])) return false
  const legacySection: Record<string, any> = document[namespace]
...
export function installLegacySettingsImport(
  ctx: any,
  config: Record<string, any>,
  namespace: string,
  entryId: string,
  ...
  if (isLegacyProvider(settingsProviderOf(ctx)) || typeof ctx?.inject !== 'function') return
