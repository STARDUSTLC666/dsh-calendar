...
export function connectionFile(env: NodeJS.ProcessEnv = process.env): string {
...
export function readConnectionFile(file: string = connectionFile()): StoredConnection {
  try {
    if (!existsSync(file)) return { revision: 0, value: {} }
...
export function writeConnectionFile(stored: StoredConnection, file: string = connectionFile()): void {
  mkdirSync(dirname(file), { recursive: true })
  const tmp = file + '.tmp'
  writeFileSync(tmp, JSON.stringify({ revision: stored.revision, value: stored.value }, null, 2) + '\n', { mode: 0o600 })
  try { chmodSync(tmp, 0o600) } catch (error) { /* Windows 上权限位无效，忽略 */ }
  renameSync(tmp, file)
}
