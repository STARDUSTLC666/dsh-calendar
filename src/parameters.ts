 ...
 */
export function compileParameters(dsl: ParameterDsl): CompiledParameters {
  const properties: Record<string, Record<string, unknown>> = {}
  const required: string[] = []
  for (const [name, spec] of Object.entries(dsl)) {
    const property: Record<string, unknown> = { type: spec.type }
  ...
  const schema: CompiledParameters = { type: 'object', properties }
