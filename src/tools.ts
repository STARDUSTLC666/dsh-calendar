...
function booleanWithDefault(args: Record<string, unknown>, key: string, fallback: boolean): boolean {
  const value = args[key]
...
function buildSearchFilter(query: string): (event: CalendarEvent) => boolean {
...
export function buildCalendarTools(config: CalendarConfig | undefined): CalendarToolDefinition[] {
  ...
  const list = {
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown): ContentBlock[] => {
        const result = value as { count: number; start: string; end: string; events: CalendarEvent[] }
        const lines = ['时间段 ' + result.start + ' ~ ' + result.end + ' 内共 ' + result.count + ' 个事件：']
        for (const event of result.events) lines.push(formatEvent(event))
  ...
  const create = {
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown): ContentBlock[] => {
        const event = (value as { created: CalendarEvent }).created
  ...
  const update = {
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown): ContentBlock[] => {
        const event = (value as { updated: CalendarEvent }).updated
    ...
    async execute(args: unknown): Promise<unknown> {
      ...
      if (start !== undefined) { assertIsoTime(start, 'start'); changes.start = start }
      const end = optionalString(input, 'end')
      if (end !== undefined) { assertIsoTime(end, 'end'); changes.end = end }
  ...
  const remove = {
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown): ContentBlock[] => {
        const result = value as { uid: string }
  ...
  const search = {
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown): ContentBlock[] => {
        const result = value as { query: string; count: number; events: CalendarEvent[] }
